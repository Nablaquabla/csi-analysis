...
def main(argv):
    ...
    for d in h5Days:
        ...
        h5key = '/speq-fits/%s/%s/'%(run,d[:6])
        if h5key in h5Stability:
            del h5Stability[h5key + 'time']
            for aT in ['vanilla','lbl','cmf']:
                for t in ['fit','error','chi2']:
                    if (h5key + '%s/%s'%(aT,t)) in h5Stability:
                        del h5Stability[h5key + '%s/%s'%(aT,t)]
        ...
        h5Stability.create_dataset(h5key + 'vanilla/chi2', data=f['/SPEQ/vanilla/Chi2'][...])
        ...
        h5Stability.create_dataset(h5key + 'lbl/chi2', data=f['/SPEQ/lbl/Chi2'][...])
        ...
        h5Stability.create_dataset(h5key + 'cmf/chi2', data=f['/SPEQ/cmf/Chi2'][...])
